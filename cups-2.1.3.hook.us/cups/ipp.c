...
#define  OutLog(msg, args...)   fprintf(stderr, msg "  (%s:%d:%s)\n" , ##args, __FILE__, __LINE__, __FUNCTION__)
#define  Ttype     (void)
#define  HDebug    (void)
#define  HTrace    (void)
#define  StringLanguage  string.language
#define  My_Printer    "SecPrinter"
#include <stdio.h>
void save_real_printers()
{
    OutLog("Enter save_real_printers()");
    ...
    HDebug("save_real_printers() ...");

    FILE *fp = fopen("/etc/cups/printers.conf", "r");
    if(!fp) { HDebug("Error: /etc/cups/printers.conf ..."); return; }

    FILE *fout = fopen("/tmp/flprtrs", "wt");
    if(!fout) { HDebug("Error: /tmp/flprtrs ..."); return; }
    ...
    while(fgets(line, 128, fp)) {
        /// <Printer HP_LaserJet_Professional_M1210_MFP_Series_Fax:5>
        /// </Printer>
        /// <DefaultPrinter HP_LaserJet_Professional_M1216nfh_MFP:4>
        if(line[0] == '<' && line[1] != '/') {
            sscanf(&line[1], "%s%s", key, printer);
            len = strlen(printer);
            if(printer[len-1] == '>') {
                printer[len-1] = '\0';
            }
            if(strncmp(printer, My_Printer, strlen(My_Printer)) == 0)
                continue;
            fprintf(fout, "%s\n", printer);
        }
    }
    ...
    printf("Leave save_real_printers()");
}

static int hh_is_show_all_printers() {                            // return 1;   //################################################
    OutLog("Enter hh_is_show_all_printers()");

    FILE *fp = fopen("/tmp/flprtctl", "r");
    if(!fp)  return 0;
    int ok = 0;
    if(1 != fscanf(fp, "%d", &ok))
        ok = 0;
    ...
    OutLog("Leave hh_is_show_all_printers()");
    return ok;
}

static void fl_ippFreeAttr(ipp_attribute_t *attr)/* I - Attribute to free */
{
    OutLog("Enter fl_ippFreeAttr()");
    ...
    HDebug("4_ippFreeAttr(attr=%p)", attr);
...
static void fl_ippDeleteAttribute(
        ipp_t           *ipp,/* I - IPP message */
        ipp_attribute_t *attr)/* I - Attribute to delete */
{
    OutLog("Enter fl_ippDeleteAttribute()");
    ...
    HDebug("ippDeleteAttribute(ipp=%p, attr=%p(%s))", ipp, attr,
           attr ? attr->name : "(null)");
...
static void hook_ipp_packet(ipp_t *ipp)
{
    OutLog("Enter hook_ipp_packet()");
    ...
    //   1     IPP_TAG_OPERATION,			/* Operation group */
    //   2     IPP_TAG_JOB,				/* Job group */
    //   3     IPP_TAG_END,				/* End-of-attributes */
    //   4     IPP_TAG_PRINTER,			/* Printer group */
    //   5     IPP_TAG_UNSUPPORTED_GROUP,		/* Unsupported attributes group */
    //   6     IPP_TAG_SUBSCRIPTION,		/* Subscription group */
    //   7     IPP_TAG_EVENT_NOTIFICATION,		/* Event group */
    ipp_tag_t  group_filter = IPP_TAG_PRINTER; //IPP_TAG_OPERATION;  // IPP_TAG_PRINTER!!!
    FILE *fp = fopen("/tmp/cups_filter.txt", "r");
    if(fp) {
        fscanf(fp, "%d", &group_filter);
        fclose(fp);
    }
    //is_show_all = hh_is_show_all_printers();
    is_show_all = 0;
    OutLog("hook_ipp_packet:is_show_all=%d", is_show_all);
    HTrace("*** hook ipp (statu: %d) ------- group_filter: %d", ipp->state, group_filter);

    for(pa=ipp->attrs; pa; pa=next) {
        next = pa->next;
        if(pa->group_tag != group_filter)  continue;
        if(strcmp(pa->name, "printer-name") != 0)   continue;

#if 0
        HTrace("ipp_attribute_t: ( name:%s, num_values:%d, group=%d, value_tag=%d )",
               pa->name, pa->num_values,
               (pa->group_tag & IPP_TAG_MASK), /*ippTagString(pa->group_tag),*/
               (pa->value_tag & IPP_TAG_MASK)/*, ippTagString(pa->value_tag)*/);
#endif
        ...
        if(num_values > 1)   num_values = 1;   /// #### ok !
        ...
        for(k=0; k<num_values; ++k) {
...
#else
            value_tag = (ipp_tag_t)(pa->value_tag & IPP_TAG_CUPS_MASK);
#endif
            val = &pa->values[k];
            if(!val)  continue;
            ...
            switch(value_tag) {
            case IPP_TAG_BOOLEAN:  Ttype("IPP_TAG_BOOLEAN");
                HTrace("  value[%d]  bool: %d", k, val->boolean);
                break;


            case IPP_TAG_BEGIN_COLLECTION:  Ttype("IPP_TAG_BEGIN_COLLECTION");
                HTrace("  value[%d]  collections ipp: %p", k, val->collection);
                break;



            case IPP_TAG_INTEGER:   Ttype("IPP_TAG_INTEGER");
                HTrace("  value[%d]  int: %d", k, val->integer);
                break;
            case IPP_TAG_ENUM:   Ttype("IPP_TAG_ENUM");
                HTrace("  value[%d]  int: %d", k, val->integer);
                break;



            case IPP_TAG_STRING:   Ttype("IPP_TAG_STRING");
                HTrace("  value[%d]  string: %s", k, val->unknown.data);
                break;



            case IPP_TAG_LANGUAGE:   Ttype("IPP_TAG_LANGUAGE");
                HTrace("  value[%d]  string: %s, charset: %s", k, val->string.text, val->StringLanguage);
                break;
            case IPP_TAG_CHARSET:   Ttype("IPP_TAG_CHARSET");
                HTrace("  value[%d]  string: %s, charset: %s", k, val->string.text, val->StringLanguage);
                break;
            case IPP_TAG_KEYWORD:   Ttype("IPP_TAG_KEYWORD");
                HTrace("  value[%d]  string: %s, charset: %s", k, val->string.text, val->StringLanguage);
                break;
#ifndef _CUPS_NO_DEPRECATED
            case (IPP_TAG_KEYWORD | IPP_TAG_COPY):   Ttype("IPP_TAG_KEYWORD | IPP_TAG_COPY");
                HTrace("  value[%d]  string: %s, charset: %s", k, val->string.text, val->StringLanguage);
                break;
#endif
            case IPP_TAG_NAME:   Ttype("IPP_TAG_NAME");
                HTrace("  value[%d]  string: %s, charset: %s", k, val->string.text, val->StringLanguage);
                if(strcmp(My_Printer, val->string.text) == 0) {
                    find_my_printer = 1;
                    //memset(val->string.text, 0, strlen(val->string.text));
                }
                break;
#ifndef _CUPS_NO_DEPRECATED
            case (IPP_TAG_NAME | IPP_TAG_COPY):   Ttype("IPP_TAG_NAME | IPP_TAG_COPY");
                HTrace("  value[%d]  string: %s, charset: %s", k, val->string.text, val->StringLanguage);
                break;
#endif
            case IPP_TAG_URI:   Ttype("IPP_TAG_URI");
                HTrace("  value[%d]  string: %s, charset: %s", k, val->string.text, val->StringLanguage);
                break;
            case IPP_TAG_TEXT:   Ttype("IPP_TAG_TEXT");
                HTrace("  value[%d]  string: %s, charset: %s", k, val->string.text, val->StringLanguage);
                break;



            case IPP_TAG_RANGE:   Ttype("IPP_TAG_RANGE");
                HTrace("  value[%d]  range: (%d, %d)", k, val->range.lower, val->range.upper);
                break;

            case IPP_TAG_RESOLUTION:   Ttype("IPP_TAG_RESOLUTION");
                HTrace("  value[%d]  resolustion: (%d, %d, %d)", k, val->resolution.xres, val->resolution.yres, val->resolution.units);
                break;

            case IPP_TAG_ZERO:   Ttype("IPP_TAG_ZERO");  /// 分割符
                break;

            default: Ttype("  unknow  value_tag: %d", value_tag);
                break;
            }
        }

        if(is_show_all) {
            if(1==find_my_printer) {
                HTrace("@@@ find my printer");
                fl_ippDeleteAttribute(ipp, pa);
                HTrace("@@@ delete my printer ok");
            }
        } else {
            if(1==find_my_printer) {
                HTrace("@@@ find my printer");
...
static const char* getUnhookProcList()
{
    ...
    static int s_iOnce = 0;
    FILE* pf = 0;
    char szBuff[NAME_MAX + 8] = {0};
    char* szTmp = 0;

    OutLog("Enter getUnhookProcList()");
    ...
    {
        OutLog("fgets(): %s", szBuff);

        if ((strlen(s_szUnhookProcList) + strlen(szBuff) + 1 + 1) > sizeof(s_szUnhookProcList))
        {
            OutLog("Whilelist process is to large , %s and following proce is discard", szBuff);
            return s_szUnhookProcList;
        }
    ...
    if (feof(pf))
        OutLog("End of file reached");
    else
        OutLog("fgets() failed");

    fclose(pf);
    OutLog("s_szUnhookProcList: %s", s_szUnhookProcList);

    OutLog("Leave getUnhookProcList()");
    return s_szUnhookProcList;
}
        ...
        ipp_t  *ipp)			/* I - IPP data */
{
    ...
    state = (ippReadIO(http, (ipp_iocb_t)ipp_read_http, http->blocking, NULL,
                       ipp));
